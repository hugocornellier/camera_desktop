...
* Fix Linux builds on distros shipping GStreamer older than 1.20, such as Ubuntu 20.04, by falling back when `gst_element_request_pad_simple` is unavailable (#10, thanks @wantroba).
...
* Add native pixel-helper unit tests (Linux and Windows) and end-to-end CI jobs that stream from a virtual red camera (v4l2loopback on Linux, a test-only fake Media Foundation source on Windows) and check the bytes are really BGRA.
